 ...
 * @returns 
 */
export function assetScaler(
    scaleOffestValues:{x:number, y:number}, 
    asset: any, 
    rel:{width:number, height:number}|null = null
){
   ...
   if (!assetScalerRecord[asset!.name]){
     ...
     */
    let twiseNewArea = 
        ((currentWindowW**2 / initSizeRation) + (currentWindowH**2 *initSizeRation))
  
    let newWidth = Math.sqrt(twiseNewArea * initSizeRation)
    let newHeight = Math.sqrt(twiseNewArea / initSizeRation)

    asset.scale.set(
        (newWidth / initWidth)*scaleOffestValues.x,
        (newHeight / initHeight)*scaleOffestValues.y
    )
}